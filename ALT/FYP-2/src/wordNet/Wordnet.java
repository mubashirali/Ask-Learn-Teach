...
public class Wordnet {
	...
	public void readIndex(String indexFile) throws IOException {
		try (BufferedReader br = new BufferedReader(new FileReader(indexFile))) {
			...
			// read copyright notice and ignore, hard-coded to ignore first 29
			// lines
			for (int i = 0; i < 29; i++)
				br.readLine();
			...
			while ((line = br.readLine()) != null) {
				...
				lemma.setTagsense_cnt(Integer
						.parseInt(columns[3 + ptr_num + 2]));

				Vector<Integer> synset_offset = new Vector<Integer>();
				for (int i = 1; i <= sense_num; i++) {
					synset_offset.add(Integer.parseInt(columns[3 + ptr_num + 2
							+ i]));
				}
		...
		try (BufferedReader br = new BufferedReader(new FileReader(synsetFile))) {
			...
			while ((line = br.readLine()) != null) {
				...
				for (int i = 1; i <= p_cnt * 4; i = i + 4) {
					...
					p.setSynset_offset(Integer.parseInt(columns[5 + w_cnt * 2
							+ i]));
					p.setPos(columns[6 + w_cnt * 2 + i]);
					p.setSource_target(Integer.parseInt(columns[7 + w_cnt * 2
							+ i], 16));
					ptr.add(p);
				}
				synset.setPtr(ptr);
